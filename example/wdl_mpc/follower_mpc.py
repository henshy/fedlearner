...
def model_fn(model, features, labels, mode):
    ...
    num_slot = 64
    x = [features['x_{0}'.format(i)] for i in range(num_slot)]
    ...
    # 通过 MPC 计算出输入层的 z，并更新权重
    nn_mpc_layer = NNMPCLayer(args.batch_size, output_dim=mpc_output_size, other_party_input_dim=input_size, model=model, mode=mode)
    result = nn_mpc_layer(embed_output)
    if mode == tf.estimator.ModeKeys.TRAIN:
        optimizer = tf.train.GradientDescentOptimizer(lr)
        train_op = model.minimize(
            optimizer, result, grad_loss=None, global_step=global_step)
        return model.make_spec(mode, loss=tf.math.reduce_mean(result),
                               train_op=train_op)
    ...
    act1_f = tf.placeholder(dtype=tf.float32, name='act1_f')
